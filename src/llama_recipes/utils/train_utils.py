        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            with profile(train_config,local_rank) as profile_context:
                for step, batch in enumerate(train_dataloader):
                    ...
                    if (train_config.output_dir.find('bubble') != -1) and (total_train_steps % 1500 == 0):
                        if train_config.save_model:
                            ...
                            if train_config.use_peft:
                                if train_config.enable_fsdp:
                                    ...
                                    print(f"we are about to save the PEFT modules")
                                model.save_pretrained(train_config.output_dir+'/checkpoint')
                                if train_config.enable_fsdp:
                                    if rank==0:
                                        print(f"PEFT modules are saved in {train_config.output_dir+'/checkpoint'} directory")
                                else:
                                    print(f"PEFT modules are saved in {train_config.output_dir+'/checkpoint'} directory")
